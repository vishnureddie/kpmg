...
<% y++;  }}%>
		...
		<div class="container">
			...
			<table class="table table-bordered subsector_list">
				...
				<tr class="table-dark">
					...
					List<HmlCategoryList> hmlc=null;
  DynamicQuery dyq1 = null;
  try {
  	dyq1 = HmlCategoryListLocalServiceUtil.dynamicQuery();
      dyq1.addOrder(OrderFactoryUtil.asc("catId"));
    //  dyq1.add(RestrictionsFactoryUtil.eq("year", Integer.toString(i)));
      hmlc = HmlCategoryListLocalServiceUtil.dynamicQuery(dyq1, 0,
  ...
  for(HmlCategoryList abt:hmlc){
	 long catId=abt.getCatId();
	 long subcatId=abt.getSubcatId();
	  String hmlCategoryInfo=abt.getHmlCategoryInfo(); 
	  if(languageId.equals("hi_IN")){
		  hmlCategoryInfo =abt.getHi_hmlCategoryInfo();
		 }
       %>
	
				<tr>
			 <% 	
	List<HmlCategoryList> hmlc2=null;
  DynamicQuery dyq2 = null;
  try {
  	dyq2 = HmlCategoryListLocalServiceUtil.dynamicQuery();
      //dyq2.addOrder(OrderFactoryUtil.asc("catId"));
     dyq2.add(RestrictionsFactoryUtil.eq("catId", catId ));
      hmlc2 = HmlCategoryListLocalServiceUtil.dynamicQuery(dyq2, 0,
    		  HmlCategoryListLocalServiceUtil.getHmlCategoryListsCount());
  } catch (Exception e) {
      e.getMessage();
  }
